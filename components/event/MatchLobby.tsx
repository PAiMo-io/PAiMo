...
export default function MatchLobby({
    ...
    const canJoinMatch = (match: MatchUI): boolean => {
        const status = getMatchStatus(match);
        const allPlayers = getAllPlayers(match);
        return status === 'waiting' && allPlayers.length < 4 && !isUserInMatch(match) && !!currentUserId;
    };

    const canStartMatch = (match: MatchUI): boolean => {
        const status = getMatchStatus(match);
        const allPlayers = getAllPlayers(match);
        return status === 'waiting' && allPlayers.length === 4 && isUserInMatch(match);
    };
    ...
    return (
        <div className='space-y-4'>
            ...
            {quickMatches.length === 0 ? (
                <Card>
                    <CardContent className='p-6 text-center text-gray-500'>{t('noQuickMatches')}</CardContent>
                </Card>
            ) : (
                                ...
                                <CardHeader className='pb-3'>
                                    <div className='flex justify-between items-center'>
                                        ...
                                        <div className='flex gap-2'>
                                            ...
                                            <Badge variant='outline'>{allPlayers.length}/4</Badge>
                                        </div>
                                    </div>
                                </CardHeader>
                                                    ...
                                                    {(match.teams?.[0]?.players || []).map((player, index) => (
                                                        <UserMiniCard
                                                            ...
                                                            user={{
                                                                ...
                                                                avatarUpdatedAt:
                                                                    (player as any).avatarUpdatedAt || null,
                                                            }}
                                                        />
                                                    ))}
                                                    {/* Team 1 Score */}
