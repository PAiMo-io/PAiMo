...
export default function MatchesScheduleSection({
    matches,
    onScoreUpdated,
    disabled = false,
                        ...
                        <TabsContent key={groupIdx} value={String(groupIdx)}>
                            <div className='grid grid-cols-1 gap-4'>
                                {groupMatches.map((match) => (
                                    <Card
                                        key={match._id}
                                        className={`overflow-visible ${!disabled ? 'cursor-pointer hover:bg-gray-50' : 'cursor-default'}`}
                                        onClick={disabled ? undefined : () => openDialog(match)}
                                    >
                                        <CardHeader>
                                            <CardTitle>
                                                Round {match.round} — Court {match.court + 1}
                                            </CardTitle>
                                        </CardHeader>
                                        <CardContent>
