export const email = {
  "register-email": {
    subject: "🎉 Bạn sắp hoàn tất! Chỉ cần xác nhận email",
    title: "Xác nhận Email của bạn",
    subTitle: "Bắt đầu bữa tiệc thôi 🎊",
    intro:
      "Chào mừng đến với <strong>PAiMO</strong>! Chỉ cần một cú nhấp chuột để mở khóa những điều tuyệt vời dành cho bạn.",
    confirmButton: "Xác nhận Email của tôi",
    fallbackTip:
      "Nếu nút không hoạt động, hãy sao chép và dán liên kết này vào trình duyệt của bạn:",
    footerNote: "Bạn nhận được email này vì đã đăng ký PAiMO.",
    copyright: "Đã đăng ký bản quyền.",
  },
  "passwordreset-email": {
    subject: "Đặt lại mật khẩu PAiMO của bạn",
    title: "Đặt lại mật khẩu",
    intro:
      "Nhấp vào nút bên dưới để đặt lại mật khẩu cho tài khoản PAiMO của bạn.",
    resetButton: "Đặt lại mật khẩu của tôi",
    fallbackTip:
      "Nếu nút không hoạt động, hãy sao chép và dán liên kết này vào trình duyệt của bạn:",
  },
  "confirm-email": {
    subject: "🎉 Bạn sắp hoàn tất! Chỉ cần xác nhận email",
    title: "Xác nhận Email của bạn",
    subTitle: "Bắt đầu bữa tiệc thôi 🎊",
    intro:
      "Chào mừng đến với <strong>PAiMO</strong>! Chỉ cần một cú nhấp chuột để mở khóa những điều tuyệt vời dành cho bạn.",
    confirmButton: "Xác nhận Email của tôi",
    fallbackTip:
      "Nếu nút không hoạt động, hãy sao chép và dán liên kết này vào trình duyệt của bạn:",
  },
  "reset-placement-email": {
    subject: "Làm lại bài khảo sát xếp hạng của bạn",
    title: "Đặt lại xếp hạng",
    subTitle: "Hãy hoàn thành lại bài khảo sát",
    intro: "Quản trị viên đã đặt lại bài khảo sát xếp hạng của bạn. Nhấn vào nút bên dưới để thực hiện lại.",
    startButton: "Làm khảo sát",
    fallbackTip: "Nếu nút không hoạt động, hãy sao chép và dán liên kết này vào trình duyệt của bạn:",
    footerNote: "Bạn nhận được email này vì xếp hạng của bạn đã được đặt lại.",
    copyright: "Đã đăng ký bản quyền.",
  },
};
