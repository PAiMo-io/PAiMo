...
export default function ClubMembersPage({ params }: { params: { id: string } }) {
    ...
    const maleCount = clubData.members.filter((m) => m.gender === 'male' || m.gender === 'Male').length;

    const femaleCount = clubData.members.filter((m) => m.gender === 'female' || m.gender === 'Female').length;

    const unknownCount = clubData.members.filter(
        (m) =>
