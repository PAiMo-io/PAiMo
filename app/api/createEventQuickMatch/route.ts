...
export async function POST(request: Request) {
    ...
    if (rematchFrom) {
        ...
        if (originalMatch && originalMatch.isQuickMatch) {
            ...
            let allPlayers = [...originalMatch.teams[0].players, ...originalMatch.teams[1].players];
    ...
    if (rematchFrom && swapPartners && playersFromRematch.length === 4) {
        // If swapping partners, create the new teams
        team1Players = [playersFromRematch[0], playersFromRematch[1]]; // e.g., [P1, P3]
        team2Players = [playersFromRematch[2], playersFromRematch[3]]; // e.g., [P2, P4]
    } else if (rematchFrom) {
        // For a regular rematch without swapping
        team1Players = [playersFromRematch[0], playersFromRematch[1]];
        team2Players = [playersFromRematch[2], playersFromRematch[3]];
    } else {
        // For a new quick match initiated by a single user
        team1Players = [session.user.id];
    }
...
export async function PUT(request: Request) {
    ...
    const isUserInMatch =
        quickMatch.teams[0].players.some((p: any) => p.toString() === userId) ||
